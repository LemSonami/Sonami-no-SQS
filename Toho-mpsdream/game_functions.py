...
def update_screen(ai_settings, screen, sb, reimu, sprites, my_bullets, 
    ...
    if QUIZ_ACTIVE:
        try:
            ai_settings._quiz_update(ai_settings)
            ai_settings._quiz_render(ai_settings, screen)
        except Exception:
            pass
...
def check_events(ai_settings, screen, stats, sb, reimu, bullets, continue_button):
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            check_keydown_events(event, ai_settings, screen, stats, reimu, bullets)
            # Quiz keys — handled separately, don't interfere with game keys
            if QUIZ_ACTIVE and event.key in (pygame.K_1, pygame.K_2, pygame.K_3, pygame.K_4):
                handler = getattr(ai_settings, "_quiz_handle_key", None)
                if handler:
                    try:
                        handler(ai_settings, event.key)
                    except Exception:
                        pass
...
def check_keydown_events(event, ai_settings, screen, stats, reimu, bullets):
    '''处理按下键的操作'''
...
def fire_boss2_barrage_1(ai_settings, screen, boss, bullets):
    '''响应第二面的琪露诺发射弹幕'''
    if boss.rect.y % 60 == 0 and boss.rect.y % 120 != 0:
